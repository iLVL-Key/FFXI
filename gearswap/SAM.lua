...
Adjusts the sets used while in Hasso stance based on the situation.

Mode1 (Multi-Attack)		-	Standard mode intended for a basic melee set.
Mode2 (Multi-Attack W/ DT)	-	Standard mode intended for a melee set with DT for survivability.
Mode3 (High Accuracy)		-	Special mode intended for a melee set with high accuracy. Weapon Skills will use the Weapon Skill - Accuracy set.
Mode4 (Subtle Blow)			-	Standard mode intended for a melee set with Subtle Blow.
...
--HUD Hasso Mode Names
	Mode1 = 'Multi-Attack',			--Standard mode.
	Mode2 = 'Multi-Attack W/ DT',	--Standard mode.
	Mode3 = 'High Accuracy',		--Special mode. NOTE: WSs will use the Weapon Skill - Accuracy set.
	Mode4 = 'Subtle Blow',			--Standard mode.
}
...
StartingHassoMode	=	'Mode1'	--[Mode1/Mode2/Mode3/Mode4]
								--	Determines the Hasso Mode you will start in. Hasso Mode can be changed at any time by using any
								--	of the three options listed above in the Notes section (a macro, alias, or keyboard shortcut).
HMBind				=	'^g'	--Sets the keyboard shortcut you would like to cycle between Hasso Modes. CTRL+G (^g) is default.
...
-- These targets automatically use the High Accuracy sets (melee and Weapon Skills), regardless of current Hasso Mode (NOT Seigan Stance).
...
Mode = StartingHassoMode --sets the starting mode (selected in the Advanced Options)
...
send_command('alias mode gs c Mode') --creates the Hasso Mode alias
send_command('alias wc gs c WC') --creates the Weapon Cycle alias
send_command('alias hud gs c HUD') --creates the HUD alias
send_command('bind '..HMBind..' gs c Mode') --creates the Hasso Mode keyboard shortcut
...
local function relicAMUpdate(tp)
	...
	local afterglow = weapon_id == 21954
	pre_AMTimer = afterglow and math.floor(tp * 0.06) or math.floor(tp * 0.02)
end

local function empyreanAMUpdate(tp)
	local weapon_id = getMainWeaponID()
	local afterglow = weapon_id == 21956
	if tp >= 3000 then
...
local function mythicAMUpdate(tp)
	local weapon_id = getMainWeaponID()
	local lvl75 = weapon_id == 18982 or weapon_id == 19002
	local lvl80_90 = weapon_id == 19071 or weapon_id == 19091 or weapon_id == 19623
...
local function setNotification()
	...
	elseif buffactive['weakness'] then
		local dto = DTOverride and " (DT Override)" or ""
		hud_noti_shdw:text('Status: Weakness'..dto)
		hud_noti:text('Status: Weakness'..dto)
		hud_noti:color(205,133,63)
	else
		local status = player.status
		local dto = DTOverride and " (DT Override)" or ""
		hud_noti_shdw:text('Status: '..status..dto)
		hud_noti:text('Status: '..status..dto)
		hud_noti:color(255,255,255)
	end
...
function choose_set()
	...
	local base_set = Stance == 'Seigan' and sets.seigan or (high_acc_mob and sets.hasso.Mode3 or sets.hasso[Mode])
	...
	elseif player.status == "Idle" then
		if AdoulinZones[world.area] then
			equip(set_combine(base_set, sets.adoulin, sets.movement_speed))
		elseif BastokZones[world.area] then
			equip(set_combine(base_set, sets.bastok, sets.movement_speed))
		elseif SandyZones[world.area] then
			equip(set_combine(base_set, sets.sandoria, sets.movement_speed))
		elseif WindyZones[world.area] then
			equip(set_combine(base_set, sets.windurst, sets.movement_speed))
...
function precast(spell)
	...
	elseif spell.type == 'WeaponSkill' then
		...
		equip(set_combine(sets.weapon_skill, ws_set, capped_attack, capped_tp, hachirin_no_obi, ygnass_resolve_1, ws_accuracy, sekkanoki, meikyo_shisui))
	...
	elseif (spell.english == 'Spectral Jig' or spell.english == 'Sneak' or spell.english == 'Monomi: Ichi' or spell.english == 'Monomi: Ni') and buffactive['Sneak'] and spell.target.type == 'SELF' then
		...
		equip(set_combine(fast_cast, main_sub))
...
function aftercast(spell)
	...
	choose_set()
	if AutoStance and StanceTimer < AutoStanceWindow and not (buffactive['amnesia'] or buffactive['impairment']) and not (TownZones[world.area] or windower.ffxi.get_info().mog_house) and not spell.interrupted then
...
windower.register_event('status change', function(status)
	...
	if AutoStance and StanceTimer < AutoStanceWindow and (status == 1 or status == 0) and not (buffactive['amnesia'] or buffactive['impairment']) and not (TownZones[world.area] or windower.ffxi.get_info().mog_house) then
		if Stance == 'Seigan' and Seigan.recast and Seigan.recast == 0 then
