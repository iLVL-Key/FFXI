...
local function getRecasts()
	...
	SoulVoice.recast = ability_recast[0] and math.ceil(ability_recast[0])
	ClarionCall.recast = ability_recast[254] and math.ceil(ability_recast[254]) or 0
	...
	Marcato.recast = ability_recast[48] and math.ceil(ability_recast[48])
	Nightingale.recast = ability_recast[109] and math.ceil(ability_recast[109])
...
local function getNameColor(input)
	...
	local lowestDuration = input and getLowestSongDuration(input)
	local color = "none"
...
local function mythicAMUpdate(tp)
	local weapon_id = getMainWeaponID()
	local lvl75 = weapon_id == 18980 or weapon_id == 19000
	local lvl80_90 = weapon_id == 19069 or weapon_id == 19089 or weapon_id == 19621
	...
	elseif tp >= 2000 and tp < 3000 then
		...
		elseif lvl80_90 then
			pre_mythicNum = math.floor(((tp * 3) / 200) - 15) --15-29
		else
			pre_mythicNum = math.floor((tp / 100) + 10) --30-39
		end
...
function choose_set()
	...
	if player.status == "Resting" then
		equip(set_combine(sets.idle, sets.rest, danger, main_sub))
	elseif player.status == "Engaged" then
		local base_set = dualWield() and sets.melee_dual_wield or sets.melee
...
function precast(spell)
	...
	elseif spell.type == 'BardSong' then
		...
		if buffactive['Nightingale'] then
			...
			--Not engaged, equip fast cast + main/sub slots for song + proper instrument
			else
...
function midcast(spell)
	...
	elseif spell.english == 'Cursna' then
		local engaged = player.status == "Engaged" and {main=pair[1],sub=pair[2]} or nil
		equip(set_combine(sets.cursna, engaged))
...
windower.register_event('action',function(act)
	...
	local actor_name = windower.ffxi.get_mob_by_id(act.actor_id) and windower.ffxi.get_mob_by_id(act.actor_id).name
