...
A simple way to quickly use Elemental Siphon.

To use the Auto Elemental Siphon function, use either of these two options:
1. A macro
	/console siphon
2. An alias command
	//siphon
...
AFTimer			=	true	--[true/false]	Displays a timer for Astral Flow in echo.
ACTimer			=	true	--[true/false]	Displays a timer for Astral Conduit in echo.
...
AutoFavor		=	true	--[true/false]	Automatically uses Favor when you summon an avatar.
AutoRelease		=	true	--[true/false]	Automatically uses Release when you summon an avatar with one already out.
...
LockWeaponOnEngage=	true	--[true/false]	Locks your Main/Sub slots when you are engaged so you can build TP for Aftermath.
...
--		Astral Flow, Astral Conduit, Apogee, Avatar's Favor, Blood Pact: Rage, Blood Pact: Ward, Convert, Dark Arts, Divine Seal, Elemental Siphon, Light Arts, Mana Cede, Stratagems, Sublimation
...
sub = {
	--SMN/WHM
	WHM = {
		Abil01 = "Blood Pact: Ward",	Abil01_sh = "BP Ward",
		Abil02 = "Blood Pact: Rage",	Abil02_sh = "BP Rage",
		Abil03 = "Apogee",				Abil03_sh = "",
		Abil04 = "Mana Cede",			Abil04_sh = "",
		Abil05 = "Elemental Siphon",	Abil05_sh = "E Siphon",
	...
	SCH = {
		...
		Abil02 = "Blood Pact: Rage",	Abil02_sh = "BP Rage",
		Abil03 = "Apogee",				Abil03_sh = "",
		Abil04 = "Elemental Siphon",	Abil04_sh = "E Siphon",
	...
	OTH = {
		...
		Abil03 = "Apogee",				Abil03_sh = "",
		Abil04 = "Elemental Siphon",	Abil04_sh = "E Siphon",
		Abil05 = "Mana Cede",			Abil05_sh = "",
		Abil06 = "Avatar's Favor",		Abil06_sh = "Avtr Favor",
	},
}
...
sets.fast_cast = {
	...
	back={ name="Campestres's Cape", augments={'Pet: M.Acc.+20 Pet: M.Dmg.+20','Eva.+20 /Mag. Eva.+20','Pet: Magic Damage+10','"Fast Cast"+10','Mag. Evasion+15',}}, --10%
}
...
--       This means that including the JP gifts, you only need a total of -20s between BPD-I and BPD-II (which each individually cap at -15s).
sets.bp_delay = {
...
AstralFlow = {} AstralConduit = {} Apogee = {} AvatarsFavor = {} BloodPactRage = {} BloodPactWard = {} Convert = {} DarkArts = {} DivineSeal = {} ElementalSiphon = {} LightArts = {} ManaCede = {} Stratagems = {}  Sublimation = {} 
...
local function getRecasts()
	...
	AstralFlow.recast = ability_recast[0] and math.ceil(ability_recast[0])
	AstralConduit.recast = ability_recast[254] and math.ceil(ability_recast[254]) or 0
	Apogee.recast = ability_recast[108] and math.ceil(ability_recast[108])
	AvatarsFavor.recast = ability_recast[176] and math.ceil(ability_recast[176])
	BloodPactRage.recast = ability_recast[173] and math.ceil(ability_recast[173])
	BloodPactWard.recast = ability_recast[174] and math.ceil(ability_recast[174])
	...
	ElementalSiphon.recast = ability_recast[175] and math.ceil(ability_recast[175])
	LightArts.recast = ability_recast[228] and math.ceil(ability_recast[228])
	ManaCede.recast = ability_recast[71] and math.ceil(ability_recast[71])
...
local function formatAbils(input,input_sh)
	...
	local ab = {} ab['Astral Flow'] = AstralFlow ab['Astral Conduit'] = AstralConduit ab['Apogee'] = Apogee ab['Avatar\'s Favor'] = AvatarsFavor ab['Blood Pact: Rage'] = BloodPactRage ab['Blood Pact: Ward'] = BloodPactWard ab['Convert'] = Convert ab['Dark Arts'] = DarkArts ab['Divine Seal'] = DivineSeal ab['Elemental Siphon'] = ElementalSiphon ab['Light Arts'] = LightArts ab['Mana Cede'] = ManaCede ab['Stratagems'] = Stratagems ab['Sublimation'] = Sublimation
...
local function relicAMUpdate(tp)
	...
	local afterglow = weapon_id == 22060
...
local function mythicAMUpdate(tp)
	local weapon_id = getMainWeaponID()
	local lvl75 = weapon_id == 18985 or weapon_id == 19005
	local lvl80_90 = weapon_id == 19074 or weapon_id == 19094 or weapon_id == 19626
	if tp >= 1000 and tp < 2000 then
		...
		elseif lvl80_90 then
			...
			pre_mythicNum = math.floor((tp / 100) + 20) --30-49
		end
...
function choose_set(has_pet)
	...
	elseif player.status == "Engaged" then
		equip(set_combine(sets.melee, main_sub))
	elseif player.status == "Idle" then
		...
		elseif TownZones[world.area] or windower.ffxi.get_info().mog_house then
			...
			equip(set_combine(base_set, danger, movement_speed))
		end
	end
...
function midcast(spell)
	...
	if spell.english == 'Phalanx' then
		equip(set_combine(sets.buff, sets.phalanx))
...
function pet_midcast(spell)
	...
	if spell.type == 'BloodPactRage' then
		if BPRagePhysical[spell.english] then
			equip(set_combine(sets.bp_rage_physical))
		elseif BPRageMagical[spell.english] then
			equip(set_combine(sets.bp_rage_magical))
		elseif BPRageHybrid[spell.english] then
			equip(set_combine(sets.bp_rage_hybrid))
		end
	elseif spell.type == 'BloodPactWard' then
		if BPWardBuff[spell.english] then
			equip(set_combine(sets.bp_ward_buff))
		elseif BPWardDebuff[spell.english] then
			equip(set_combine(sets.bp_ward_debuff))
		end
	end
...
function aftercast(spell)
	...
	if spell.english == 'Astral Flow' and AFTimer and not spell.interrupted then
...
windower.register_event('prerender', function()
	...
	if clock - last_poll >= 1 / PollingRate then
		...
		elseif not pet.isvalid and PetPresent then --we do not have an avatar out when we previously did (ie releasing or they die)
			PetPresent = false
