...
Mode5 (Tank)				-	Standard mode intended for a melee set with counter and DT for tanking.
...
StartingGearMode	=	'Mode1'	--[Mode1/Mode2/Mode3/Mode4]
...
GMBind				=	'^g'	--Sets the keyboard shortcut you would like to cycle between Geaar Modes. CTRL+G (^g) is default.
...
-- These targets automatically use the High Accuracy sets (melee and Weapon Skills), regardless of current Gear Mode (except Tank).
...
local function relicAMUpdate(tp)
	...
	local afterglow = weapon_id == 21683 or weapon_id == 21756
	pre_AMTimer = afterglow and math.floor(tp * 0.06) or math.floor(tp * 0.02)
end

local function empyreanAMUpdate(tp)
	local weapon_id = getMainWeaponID()
	local afterglow = weapon_id == 21752 or weapon_id == 21758
	if tp >= 3000 then
...
local function mythicAMUpdate(tp)
	local weapon_id = getMainWeaponID()
	local lvl75 = weapon_id == 18971 or weapon_id == 18991
	local lvl80_90 = weapon_id == 19060 or weapon_id == 19080 or weapon_id == 19612
...
function choose_set()
	...
	local base_set = twoHanded() and (high_acc_mob and sets.Mode3.two_handed or sets[Mode].two_handed) or (dualWield() and (high_acc_mob and sets.Mode3.dual_wield or sets[Mode].dual_wield) or (high_acc_mob and sets.Mode3.single_wield or sets[Mode].single_wield))
...
function aftercast(spell)
	...
	choose_set()
	if AutoStance and twoHanded() and StanceTimer < AutoStanceWindow and player.sub_job == 'SAM' and not (buffactive['amnesia'] or buffactive['impairment']) and not spell.interrupted and not (TownZones[world.area] or windower.ffxi.get_info().mog_house) then
...
windower.register_event('status change', function(status)
	...
	if AutoStance and twoHanded() and StanceTimer < AutoStanceWindow and player.sub_job == 'SAM' and (status == 1 or status == 0) and not (buffactive['amnesia'] or buffactive['impairment']) and not (TownZones[world.area] or windower.ffxi.get_info().mog_house) then
		if Stance == 'Seigan' and Seigan.recast and Seigan.recast == 0 then
