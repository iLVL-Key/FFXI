...
Tank	-	Equips gear sets as if you are always actively tanking.
Kite	-	Equips gear sets as if you are always actively kiting.
Neutral	-	Equips gear sets as if you are always neutral (not in battle).
DPS		-	Equips gear sets as if you are not tanking, only DD'ing.
...
AutoMvmntSpeed	=	true	--[true/false]	Automatically equips Movement Speed set on player movement when idle and not in combat.
...
StartingGearMode	=	'Auto'	--[Auto/Tank/Kite/Neutral/DPS]
...
local function mythicAMUpdate(tp)
	local weapon_id = getMainWeaponID()
	local lvl75 = weapon_id == 18977 or weapon_id == 18997
	local lvl80_90 = weapon_id == 19066 or weapon_id == 19086 or weapon_id == 19618
...
function choose_set()
	...
	elseif player.status == "Idle" then
		...
		elseif TownZones[world.area] or windower.ffxi.get_info().mog_house then
			...
			local base_set = (auto_combat or Mode == "Kite" or Mode == "Tank") and (TankMDTTriggerFound() and sets.tank_mdt or sets.tank) or sets.idle
			...
			local movement_speed = (auto_combat or Mode == "Kite" or auto_movement_speed or autorun) and sets.movement_speed or nil
			equip(set_combine(base_set, max_hp, low_hp, movement_speed, {main=pair[1],sub=pair[2]}))
		end
	end
...
function precast(spell)
	...
	elseif spell.english == "Flash" then
		...
		elseif player.sub_job == 'WAR' and player.sub_job_level ~= 0 then
			if windower.ffxi.get_ability_recasts()[5] and windower.ffxi.get_ability_recasts()[5] < 2 and spell.target.distance <= 17.8 and not (buffactive['amnesia'] or buffactive['impairment']) then
	...
	elseif spell.english == "Cure IV" then
		...
		local cures = {
			...
			{name="Cure III",	mp=math.floor(46 * la)},
...
function aftercast(spell)
	...
	if AutoDefender and (spell.english == 'Provoke' or spell.english == 'Holy Circle' or spell.english == 'Shield Bash' or spell.english == 'Sentinel' or spell.english == 'Rampart') and not (buffactive['Defender'] or buffactive['amnesia'] or buffactive['impairment']) and (Mode == 'Tank' or Mode == 'Kite' or Mode == 'Auto') and player.sub_job == 'WAR' and player.sub_job_level ~= 0 and DefenderRecast == 0 then
...
windower.register_event('prerender', function()
	...
	if current_time - last_poll >= 1 / PollingRate then
		...
		if AutoSentinel and player.hp <= AutSntThreshold and Alive and not (buffactive['Weakness'] or buffactive['amnesia'] or buffactive['impairment'] or buffactive['terror'] or buffactive['petrification'] or buffactive['sleep']) and Sentinel.recast and Sentinel.recast == 0 and not (TownZones[world.area] or windower.ffxi.get_info().mog_house) then
...
windower.register_event('action',function(act)
	...
	elseif (act.category == 15 and msg == 110 and ata.unknown == 4) and act.actor_id == player.id then
